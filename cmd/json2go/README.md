...
Struct tags with the key `json` are generated for all fields.  Additional struct tags can be generated by using the `-tagkey` or `-t` flags.  Lists of keys are supported by passing the `-tagkey`, or `-t` flag for each key value:

    json2g0 -o example.go -t xml -t db
